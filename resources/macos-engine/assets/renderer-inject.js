((cssText, artDataUrl, pixelCatDataUrl, themeConfig, effectsConfig) => {
  const STATE_KEY = "__CODEX_DREAM_SKIN_STATE__";
  const DISABLED_KEY = "__CODEX_DREAM_SKIN_DISABLED__";
  const STYLE_ID = "codex-dream-skin-style";
  const CHROME_ID = "codex-dream-skin-chrome";
  const WINDOW_FRAME_ID = "codex-dream-skin-window-frame";
  const PIXEL_CAT_ID = "codex-dream-skin-pixel-cat";
  ...
  const EFFECTS = effectsConfig && typeof effectsConfig === "object" ? effectsConfig : {};
  const PIXEL_CAT_ASSETS = pixelCatDataUrl && typeof pixelCatDataUrl === "object"
    ? pixelCatDataUrl
    : { dark: pixelCatDataUrl, light: pixelCatDataUrl };
  ...
  const THEME_VARIABLES = [
    ...
    "--dream-pixel-cat-light", "--dream-pixel-cat-dark",
  ];
  const installToken = {};
  ...
  if (previous?.visibilityHandler && typeof document.removeEventListener === "function") {
    document.removeEventListener("visibilitychange", previous.visibilityHandler);
  }
  if (previous?.navigationHandler && previous?.navigation) {
    try { previous.navigation.removeEventListener("navigate", previous.navigationHandler); } catch {}
  }
  ...
  const applyRootState = (root) => {
    ...
    setStyleProperty(root, "--dream-pixel-cat-light", `url("${PIXEL_CAT_ASSETS.light || ""}")`);
    setStyleProperty(
      root,
      "--dream-pixel-cat-dark",
      `url("${PIXEL_CAT_ASSETS.dark || PIXEL_CAT_ASSETS.light || ""}")`,
    );
  ...
  const createCatController = () => {
    ...
    const bounds = () => {
      ...
      const left = Math.max(20, Math.round((box?.left ?? 0) + 28));
      const right = Math.max(left, Math.round(Math.min(box?.right ?? width, width) - 92));
      const top = Math.max(72, Math.round((box?.top ?? 0) + 68));
      const bottom = Math.max(top, Math.round(Math.min(box?.bottom ?? height, height) - 168));
      return { left, right, top, bottom };
    };
    const placeResting = () => {
      if (!current) return;
      const area = bounds();
      current.classList.remove("is-walking");
      current.style.transition = "none";
      current.style.transform = `translate3d(${area.right}px, ${area.bottom}px, 0)`;
    };
    const move = () => {
      if (disposed || !current || !current.isConnected) return;
      ...
      const area = bounds();
      const previousX = Number(current.dataset.x ?? area.right);
      const previousY = Number(current.dataset.y ?? area.bottom);
      const x = Math.round(area.left + Math.random() * Math.max(1, area.right - area.left));
      const y = Math.round(area.top + Math.random() * Math.max(1, area.bottom - area.top));
      const distance = Math.hypot(x - previousX, y - previousY);
      const duration = Math.round(Math.min(24000, Math.max(6000, distance / 0.032)));
      current.dataset.x = String(x);
      current.dataset.y = String(y);
      current.classList.toggle("faces-left", x < previousX);
      current.classList.add("is-walking");
      current.style.transition = `transform ${duration}ms linear`;
      frame = requestAnimationFrame(() => {
        frame = null;
        if (current) current.style.transform = `translate3d(${x}px, ${y}px, 0)`;
      });
      timer = setTimeout(() => {
        timer = null;
        if (current && !disposed && document.visibilityState !== "hidden") {
          timer = setTimeout(move, 80);
        }
      }, duration);
    };
    const attach = (cat, shellMain, relayout = false) => {
      current = cat;
      main = shellMain;
      if (!reducedQuery) {
        try {
          reducedQuery = window.matchMedia("(prefers-reduced-motion: reduce)");
          reducedQuery.addEventListener("change", relayoutNow);
        } catch {}
      }
      if (!cat.dataset.x) {
        const area = bounds();
        cat.dataset.x = String(area.right);
        cat.dataset.y = String(area.bottom);
        cat.style.transform = `translate3d(${area.right}px, ${area.bottom}px, 0)`;
        if (document.visibilityState !== "hidden") timer = setTimeout(move, 120);
      } else if (relayout) {
        relayoutNow();
      }
    };
    function relayoutNow() {
      clearPending();
      if (reducedQuery?.matches) placeResting();
      else if (document.visibilityState !== "hidden") timer = setTimeout(move, 120);
    }
    const pause = () => {
      clearPending();
      current?.classList.remove("is-walking");
    ...
    const resume = () => {
      if (disposed || reducedQuery?.matches || document.visibilityState === "hidden") return;
      clearPending();
      timer = setTimeout(move, 120);
    };
    const dispose = () => {
      disposed = true;
      clearPending();
      try { reducedQuery?.removeEventListener("change", relayoutNow); } catch {}
      document.getElementById(PIXEL_CAT_ID)?.remove();
    };
    return { attach, dispose, pause, relayout: relayoutNow, resume };
  };
  ...
  const syncRouteState = (shell, { layout = false } = {}) => {
    ...
    let windowFrame = document.getElementById(WINDOW_FRAME_ID);
    if (EFFECTS.effectsEnabled !== false && EFFECTS.windowBorderEnabled) {
      if (!windowFrame || windowFrame.parentElement !== document.body) {
        windowFrame?.remove();
        windowFrame = document.createElement("div");
        windowFrame.id = WINDOW_FRAME_ID;
      ...
      windowFrame.setAttribute(
        ...
        ["classic-rainbow", "candy-stripe", "ocean", "monochrome"]
          .includes(EFFECTS.windowBorderStyle)
    ...
    let pixelCat = document.getElementById(PIXEL_CAT_ID);
    if (EFFECTS.effectsEnabled !== false && EFFECTS.pixelCatEnabled) {
      if (!pixelCat || pixelCat.parentElement !== document.body) {
        pixelCat?.remove();
        pixelCat = document.createElement("div");
        pixelCat.id = PIXEL_CAT_ID;
        pixelCat.setAttribute("aria-hidden", "true");
        pixelCat.innerHTML = '<i class="dream-pixel-cat-sprite"></i>';
        document.body.appendChild(pixelCat);
      }
      catController.attach(pixelCat, shellMain, layout);
    } else {
      pixelCat?.remove();
    }
  ...
  const cleanup = () => {
    ...
    document.documentElement?.removeAttribute("data-dream-document-hidden");
    ...
    document.getElementById(WINDOW_FRAME_ID)?.remove();
    document.getElementById(PIXEL_CAT_ID)?.remove();
  ...
  const scheduleEnsure = ({ root = false, route = true, layout = false } = {}) => {
    ...
    // Codex streams many small DOM mutations while generating. Re-running the
    // route scan on the next animation frame for every batch makes the skin
    // compete with the renderer. A short trailing debounce keeps navigation
    // responsive without turning token streaming into a full-layout loop.
    scheduler.timeout = setTimeout(flushScheduledEnsure, 300);
  };
  ...
  const containsRouteStructure = (node) => {
    if (!node || node.nodeType !== 1) return false;
    try {
      return node.matches(structuralSelector) || Boolean(node.querySelector(structuralSelector));
    } catch {
      return false;
    }
  };
  const observer = new MutationObserver((mutations) => {
    const structural = mutations.some((mutation) =>
      [...mutation.addedNodes, ...mutation.removedNodes].some(containsRouteStructure));
    if (structural) scheduleEnsure({ route: true });
  });
  ...
  const visibilityHandler = () => {
    ...
    scheduleEnsure({ root: true, route: true, layout: true });
  };
  ...
  setAttribute(document.documentElement, "data-dream-document-hidden", document.visibilityState === "hidden");
  const timer = setInterval(() => {
    if (document.visibilityState !== "hidden") ensure({ layout: false });
  }, 30000);
  ...
  __DREAM_SKIN_PIXEL_CAT_JSON__,
  __DREAM_SKIN_THEME_JSON__,
  __DREAM_SKIN_EFFECTS_JSON__
)
