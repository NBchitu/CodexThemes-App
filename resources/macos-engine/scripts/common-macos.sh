...
WINDOW_EFFECTS_PATH="$STATE_ROOT/window-effects.json"
      ...
      if [ "$token_guarded" = "true" ] && [ -f "$OPERATION_STATE_PATH" ]; then
        ...
        if [ "$current_token" != "$operation_token" ]; then
          if [ "$terminal_policy" = "idle" ]; then
            ...
            case "$current_status" in applying) current_ttl=90 ;; pausing) current_ttl=60 ;; *) current_ttl=0 ;; esac
...
verify_macos_app_signature() {
  ...
  if [ "$verification_mode" = "deep" ]; then
    # Some official Codex builds contain nested login/update components whose
    # signing requirements differ from the main app. Prefer the strongest
    # whole-tree verification when it succeeds, then fall back to verifying
    # the exact signed bundle and executable that this runtime launches.
    if /usr/bin/codesign --verify --deep --strict \
      --test-requirement "=$EXPECTED_CODEX_REQUIREMENT" "$CODEX_BUNDLE" >/dev/null 2>&1; then
      return 0
    fi
  fi
  # A valid official update can contain nested helpers whose resource seals no
  # longer satisfy a whole-bundle fallback check. The signed Node runtime and
  # Team ID were already verified above; verify the exact executable we launch
  # here instead of accepting an unsigned or foreign binary.
  /usr/bin/codesign --verify --strict \
    --ignore-resources \
    --test-requirement "=$EXPECTED_CODEX_REQUIREMENT" "$CODEX_EXE" >/dev/null 2>&1 \
    || fail "The Codex executable signature is not valid. Restore or reinstall the official app before continuing."
}
...
codex_main_pids() {
  ...
  while read -r process_uid pid command_line; do
    [ -n "$pid" ] || continue
...
launch_injector_daemon() {
  ...
  if /bin/launchctl submit -l "$INJECTOR_JOB_LABEL" -o "$INJECTOR_LOG" -e "$INJECTOR_ERROR_LOG" -- \
    ...
    --effects-config "$WINDOW_EFFECTS_PATH" \
...
hot_reapply_theme() {
  ...
  if ! "$NODE" "$INJECTOR" --once --port "$port" --theme-dir "$THEME_DIR" \
    ...
    write_operation_state failed "应用失败，显示效果未确认" "$operation_token" 2>/dev/null || true
    return 1
  fi

  # Restart the watcher after a successful one-shot apply. The executable path
  # can stay the same across App upgrades while the already-running Node
  # process still holds the previous CSS/template in memory.
  if ! stop_recorded_injector 2>/dev/null; then
    write_operation_state failed "应用失败，显示效果未确认" "$operation_token" 2>/dev/null || true
    return 1
  fi
  ...
  if ! write_state "$port" "$inj_pid" "$started_at" "${codex_pid:-0}" active; then
    write_operation_state failed "应用失败，显示效果未确认" "$operation_token" 2>/dev/null || true
    return 1
  fi
  if ! write_operation_state success "皮肤已应用" "$operation_token"; then
    return 1
  fi
  return 0
}
