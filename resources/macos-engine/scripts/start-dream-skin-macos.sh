...
record_start_exit() {
  ...
  [ -z "${VERIFY_ERROR:-}" ] || /bin/rm -f "$VERIFY_ERROR"
  [ "$code" -ne 0 ] || return 0
...
# Do not use -n here: a second instance can race ChatGPT's reopen handler and
# leave a debuggable renderer without a native window.
activate_codex_window
...
VERIFY_ERROR="$(/usr/bin/mktemp "${TMPDIR:-/tmp}/dream-skin-verify-error.XXXXXX")"
/bin/chmod 600 "$VERIFY_OUTPUT"
/bin/chmod 600 "$VERIFY_ERROR"
...
if "$NODE" "$INJECTOR" --verify --port "$PORT" --theme-dir "$THEME_DIR" --timeout-ms 6000 >"$VERIFY_OUTPUT" 2>"$VERIFY_ERROR"; then
  verify_code=0
else
  verify_code=$?
fi
if [ "$verify_code" -ne 0 ]; then
  ...
  if [ -n "$OPERATION_TOKEN" ]; then
    "$NODE" "$INJECTOR" --once --port "$PORT" --theme-dir "$THEME_DIR" --timeout-ms 6000 \
      --operation-token "$OPERATION_TOKEN" >/dev/null 2>&1 || true
  else
    "$NODE" "$INJECTOR" --once --port "$PORT" --theme-dir "$THEME_DIR" --timeout-ms 6000 >/dev/null 2>&1 || true
  fi
  if "$NODE" "$INJECTOR" --verify --port "$PORT" --theme-dir "$THEME_DIR" --timeout-ms 4000 >"$VERIFY_OUTPUT" 2>"$VERIFY_ERROR"; then
    verify_code=0
  else
    verify_code=$?
  fi
...
if [ "$verify_code" -ne 0 ]; then
  ...
  if /usr/bin/grep -q 'Codex is not signed in or has not finished loading' "$VERIFY_ERROR" 2>/dev/null; then
    login_required="true"
  fi
  ...
  if [ "$login_required" = "true" ]; then
    fail "Codex is showing the sign-in screen or its workspace is not ready. Sign in to Codex, wait for the main workspace to appear, then apply the theme again."
  fi
