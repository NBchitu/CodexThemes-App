...
if [ -f "$OPERATION_STATE_PATH" ]; then
  ...
  operation_token="$(read_plist_snapshot_field "$OPERATION_SNAPSHOT" operationToken)"
  operation_updated_at="$(read_plist_snapshot_field "$OPERATION_SNAPSHOT" updatedAt)"
  operation_pid="${operation_token%%:*}"
  now="$(/bin/date +%s)"
  case "$operation_updated_at" in ''|*[!0-9]*) operation_updated_at="0" ;; esac
  case "$operation_pid" in ''|*[!0-9]*) operation_pid="0" ;; esac
  ...
  if { [ "$operation_status" = "applying" ] || [ "$operation_status" = "pausing" ]; } \
    && { [ "$operation_pid" -le 1 ] || ! /bin/kill -0 "$operation_pid" 2>/dev/null; }; then
    if [ "$SESSION" = "active" ] && [ "$INJECTOR_ALIVE" = "true" ]; then
      OPERATION_STATUS=""
      OPERATION_MESSAGE=""
    else
      OPERATION_STATUS="failed"
      OPERATION_MESSAGE="上次操作已中断，请重试"
    fi
  elif [ "$age" -ge 0 ] && [ "$ttl" -gt 0 ] && [ "$age" -le "$ttl" ]; then
