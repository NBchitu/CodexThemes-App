...
# Verification requires the real renderer to be visible. Activate the already
# running, identity-bound app without `-n`; this never creates a second Codex
# instance and avoids treating a background renderer as successfully applied.
/usr/bin/open -a "$CODEX_BUNDLE" >/dev/null 2>&1 || true

# Hot path: CDP already open → seconds, not tens of seconds
if hot_reapply_theme "$PORT" 5000 "$OPERATION_TOKEN"; then
